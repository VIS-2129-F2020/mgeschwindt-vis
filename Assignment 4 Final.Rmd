...
```{r assign projection, message=FALSE, results='hide'}
...
```{r street map test plot, fig.height=10, fig.width=10}
ggplot(nyc_streets) +
  geom_sf() +
  theme_map()
```
...
```{r call otp path, message=FALSE}
path_otp <- otp_dl_jar("OTP")
```

```{r call data, message=FALSE, results='hide'}
...
```{r define isochrones, message=FALSE, results='hide'}
iso_10min_walk <- 
...
```{r isochrone 1, fig.height=10, fig.width=10}
...
```{r isochrone 2, fig.height=10, fig.width=10}
ggplot(iso_all_modes) +
...
```{r isochrone 3, fig.height=10, fig.width=10}
ggplot(iso_all_modes) +
...
```{r stacked area chart, message=FALSE, fig.width=12}
...
```{r otp stop, message=FALSE, results='hide'}
otp_stop()
```
